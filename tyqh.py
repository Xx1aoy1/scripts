...
#
# 环境变量：
#   qiehuang_ck：账号凭证，格式为 wid----openId，多账号用 & 分隔
#
# 示例：qiehuang_ck=wid值----openId值
...
def main():
    ...
    if CRYPTO_BACKEND is None:
        ...
        return
    if not users:
        message = "没有可用账号：未读取到 qiehuang_ck 环境变量，格式：wid----openId"
        print(message)
        safe_send("统一茄皇五期", message)
        return
